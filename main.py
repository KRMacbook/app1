...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    CUSTOMER main Traveller_type: {traveller_type};
    TASK: Write a product description that is tailored into this customer's Age group and traveller type. Use age group specific slang.;
    ...
    BENEFITS: describe in 3 sentences why this product is perfect considering customers age group and traveller type;
    USE CASE: write a story in 5 sentences, of an example weekend activity taking into account traveller type {traveller_type} and age {agegroup}, write a story in first person, example "I started my Saturday morning with ..."; OUTPUT TEXT in Estonian;
"""

prompt = PromptTemplate(
    input_variables=["agegroup", "traveller_type", "content"],
    template=template,
)
...
with col1:
    ...
    \n\n Kasutusjuhend: 1) valmista ette tootekirjeldus (sisendtekst). 2) määra tarbijasegemendid lähtuvalt vanuserühma ja reisija tüübi kombinatsioonidest. 3) sisesta ükshaaval tarbijasegmentide lõikes eeltoodud info äpi kasutajaliideses, saada ära. \
    4) kopeeri ükshaaval tarbijasegmentide lõikes äpi väljundteksti kõnealuse toote tutvustuslehele.")

with col2:
    st.image(image='Unknown', caption='Estravel')
...
def get_hobby():
    input_text = st.text_input(label="Customers main traveller type", key="hobby_input")
    return input_text
...
#        st.stop()
    ...
    prompt_with_content = prompt.format(agegroup=option_agegroup, traveller_type=hobby_input, content=content_input)
